...
<div id='text_paynow_status_map'>
	...
	<div class="control-group">
		<label class='control-label' for="paynow_canceled">{__("cancelled")}:</label>
		<div class="controls">
			<select name="payment_data[processor_params][statuses][canceled]" id="paynow_canceled">
				{foreach from=$statuses item="s" key="k"}
				<option value="{$k}" {if (isset($processor_params.statuses.canceled) && $processor_params.statuses.canceled == $k) || (!isset($processor_params.statuses.canceled) && $k == 'F')}selected="selected"{/if}>{$s}</option>
				{/foreach}
			</select>
		</div>
	</div>
	...
	<div class="control-group">
		<label class='control-label' for="paynow_declined">{__("declined")}:</label>
		<div class="controls">
			<select name="payment_data[processor_params][statuses][declined]" id="paynow_declined">
				{foreach from=$statuses item="s" key="k"}
				<option value="{$k}" {if (isset($processor_params.statuses.declined) && $processor_params.statuses.declined == $k) || (!isset($processor_params.statuses.declined) && $k == 'F')}selected="selected"{/if}>{$s}</option>
				{/foreach}
			</select>
		</div>
	</div>
